...
public class SingleThread {
    ...
    public SingleThread(GoogleScenario googleScenario, YouTubeScenario youtubeScenario, YoutubeDatabases youtubeDatabases) {
...
//                googleScenario.goGoogleSignInPage();
                ...
                for (YoutubeChannel channel : channels) {
                    ...
                    videos.forEach(youtubeScenario::openLink);
                }
...
//                googleScenario.attemptToSignOutYouTube();
    ...
    public static void main(String[] args) {
        ...
        SingleThread SingleThread = new SingleThread(googleScenario, youtubeScenario, youtubeDatabases);

        try {
            SingleThread.playScenario4();
        } catch (Exception e) {
            e.printStackTrace();
            severe("Scenario suspend unexpectedly");
            severe("Number attempt" + SingleThread.numberAttempt);
            severe("Failed account " + SingleThread.failedAccount);
        } finally {
            DriverUtil.close();
        }
    }
